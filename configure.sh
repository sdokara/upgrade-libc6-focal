...
echo "Injecting hirsute repositories..."
sudo tee -a /etc/apt/sources.list >/dev/null <<EOF
deb http://ba.archive.ubuntu.com/ubuntu/ hirsute main restricted
deb http://ba.archive.ubuntu.com/ubuntu/ hirsute-updates main restricted
EOF
sudo apt-get update


echo -n "Evaluating dependencies to upgrade... "
...
sudo sed -i '/^deb.*hirsute.*$/d' /etc/apt/sources.list
sudo apt-get update
EOF
